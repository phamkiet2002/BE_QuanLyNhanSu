...
public sealed class GetLeaveRegistrationTypeOfLeaveDayOffQueryHandler : IQueryHandler<Query.GetLeaveRegistrationTypeOfLeaveDayOffsQuery, PagedResult<Response.LeaveRegistrationTypeOfLeaveDayOffResponse>>
{
    private readonly IMapper _mapper;
    private readonly IRepositoryBase<Domain.Entities.LeaveRegistration, Guid> _leaveRegistrationRepository;

    public GetLeaveRegistrationTypeOfLeaveDayOffQueryHandler(IMapper mapper, IRepositoryBase<Domain.Entities.LeaveRegistration, Guid> leaveRegistrationRepository)
    {
        _mapper = mapper;
        _leaveRegistrationRepository = leaveRegistrationRepository;
    }

    public async Task<Result<PagedResult<Response.LeaveRegistrationTypeOfLeaveDayOffResponse>>> Handle(Query.GetLeaveRegistrationTypeOfLeaveDayOffsQuery request, CancellationToken cancellationToken)
    {
        ...
        var query = _leaveRegistrationRepository.FindAll(x => x.TypeOfLeave == Domain.Enumerations.LeaveRegistrationEnums.TypeOfLeave.Nghingay & x.Employee.Status == Status.Active);
        ...
        var result = _mapper.Map<PagedResult<Response.LeaveRegistrationTypeOfLeaveDayOffResponse>>(leaveRegistrationPagedResult);
